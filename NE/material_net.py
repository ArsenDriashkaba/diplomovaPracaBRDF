...
def _parse_function(filename):
    ...
    def transform(image):
        ...
        r = tf.image.resize(r, [args.scale_size, args.scale_size], method=tf.image.ResizeMethod.AREA)
        return r
...
def load_examples(input_dir, shuffleValue):
    ...
    with tf.name_scope("load_images"):
        ...
        # iterator = tf.data.make_initializable_iterator(batched_dataset)

        iterator = tf.compat.v1.data.Iterator.from_structure(tf.compat.v1.data.get_output_types(batched_dataset),
                                           tf.compat.v1.data.get_output_shapes(batched_dataset))

        paths_batch, inputs_batch, targets_batch = iterator.get_next()
        
        if args.scale_size > CROP_SIZE:
            xyCropping = tf.random.uniform([2], 0, args.scale_size - CROP_SIZE, dtype=tf.int32)
