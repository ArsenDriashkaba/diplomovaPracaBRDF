...
parser.add_argument("--mode", required=True, choices=["test", "train", "eval", "eval_lite"])
...
def instancenorm(input):
    with tf.compat.v1.variable_scope("instancenorm"):
        ...
        offset = tf.compat.v1.get_variable("offset", [1, 1, 1, channels], dtype=tf.float32, initializer=tf.compat.v1.zeros_initializer())
        scale = tf.compat.v1.get_variable("scale", [1, 1, 1, channels], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(1.0, 0.02))
...
def deconv(batch_input, out_channels):
   with tf.compat.v1.variable_scope("deconv"):
        ...
        filter = tf.compat.v1.get_variable("filter", [4, 4, in_channels, out_channels], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(0, 0.02))
        filter1 = tf.compat.v1.get_variable("filter1", [4, 4, out_channels, out_channels], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(0, 0.02))
        ...
        conv = tf.nn.conv2d(input=resized_images, filters=filter, strides=[1, 1, 1, 1], padding="SAME")
        conv = tf.nn.conv2d(input=conv, filters=filter1, strides=[1, 1, 1, 1], padding="SAME")

        #conv = tf.nn.conv2d_transpose(batch_input, filter, [batch, in_height * 2, in_width * 2, out_channels], [1, 2, 2, 1], padding="SAME")
        return conv

def tf_generate_normalized_random_direction(batchSize, lowEps = 0.001, highEps =0.05):
    r1 = tf.random.uniform([batchSize, 1], 0.0 + lowEps, 1.0 - highEps, dtype=tf.float32)
    r2 =  tf.random.uniform([batchSize, 1], 0.0, 1.0, dtype=tf.float32)
    r = tf.sqrt(r1)
    phi = 2 * math.pi * r2
...
def tf_generate_distance(batchSize):
    gaussian = tf.random.normal([batchSize, 1], 0.5, 0.75, dtype=tf.float32) # parameters chosen empirically to have a nice distance from a -1;1 surface.
...
def _parse_function(filename):
    ...
    with tf.control_dependencies([assertion]):
        ...
        width = tf.shape(input=input)[1] # [height, width, channels]
...
def load_examples(input_dir, shuffleValue):
    ...
    with tf.compat.v1.name_scope("load_images"):
        ...
        # paths_batch, inputs_batch, targets_batch = next(iterator)
        paths_batch, inputs_batch, targets_batch = iterator.get_next()
        
        if a.scale_size > CROP_SIZE:
            xyCropping = tf.random.uniform([2], 0, a.scale_size - CROP_SIZE, dtype=tf.int32)
    ...
    tf.compat.v1.summary.text("batch paths", paths_batch)
...
def fullyConnected(input, outputDim, useBias, layerName = "layer", initMultiplyer = 1.0):
    with tf.compat.v1.variable_scope("fully_connected"):
        batchSize = tf.shape(input=input)[0];
        inputChannels = int(input.get_shape()[-1])
        weights = tf.compat.v1.get_variable("weight", [inputChannels, outputDim ], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(0, initMultiplyer * tf.sqrt(1.0/float(inputChannels))))
        ...
        if(useBias):
            bias = tf.compat.v1.get_variable("bias", [outputDim], dtype=tf.float32, initializer=tf.compat.v1.random_normal_initializer(0, 0.002))
            outputs = outputs + tf.expand_dims(bias, axis = 0)
            
        return outputs

def GlobalToGenerator(inputs, channels):
    with tf.compat.v1.variable_scope("GlobalToGenerator1"):
...
def create_generator(generator_inputs, generator_outputs_channels):
    ...
    inputMean, inputVariance = tf.nn.moments(x=generator_inputs, axes=[1, 2], keepdims=False)
    globalNetworkInput = inputMean
    globalNetworkOutputs = []
    with tf.compat.v1.variable_scope("globalNetwork_fc_1"):    
    ...
    for layerCount, out_channels in enumerate(layer_specs):
        with tf.compat.v1.variable_scope("encoder_%d" % (len(layers) + 1)):
            ...
            with tf.compat.v1.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):  
    ...
    with tf.compat.v1.variable_scope("encoder_8"):
        rectified = lrelu(layers[-1], 0.2)
         # [batch, in_height, in_width, in_channels] => [batch, in_height/2, in_width/2, out_channels]
        ...
        with tf.compat.v1.variable_scope("globalNetwork_fc_%d" % (len(globalNetworkOutputs) + 1)):  
            mean, variance = tf.nn.moments(x=convolved, axes=[1, 2], keepdims=True)
            nextGlobalInput = tf.concat([tf.expand_dims(tf.expand_dims(globalNetworkOutputs[-1], axis = 1), axis=1), mean], axis = -1)
...
def tf_generateSpecularRendering(batchSize, surfaceArray, targets, outputs):    
    ...
    currentShift = tf.concat([tf.random.uniform([batchSize, 2], -1.0, 1.0), tf.zeros([batchSize, 1], dtype=tf.float32) + 0.0001], axis=-1)
...
def create_model(inputs, targets, reuse_bool = False):
    batchSize = tf.shape(input=inputs)[0]
    surfaceArray=[]
    if a.loss == "render" or a.loss == "renderL2":
        XsurfaceArray = tf.expand_dims(tf.linspace(-1.0, 1.0, CROP_SIZE), axis=-1)
        XsurfaceArray = tf.tile(XsurfaceArray,[1,CROP_SIZE])
        YsurfaceArray = -1 * tf.transpose(a=XsurfaceArray) #put -1 in the bottom of the table
    ...
    with tf.compat.v1.variable_scope("generator", reuse=reuse_bool) as scope:
        out_channels = 9
    ...
    normalShape = tf.shape(input=partialOutputedNormals)
    ...
    with tf.compat.v1.name_scope("generator_loss"):
        ...
        if a.loss == "l1":
            ...
            DiffuseL1 = tf.abs(tf.math.log(epsilon + deprocess(targets[0,:,:,3:6])) - tf.math.log(epsilon + deprocess(outputs[0,:,:,3:6]))) * a.diffuseLossFactor
            RoughnessL1 = tf.abs(targets[0,:,:,6:9] - outputs[0,:,:,6:9]) * a.roughnessLossFactor
            SpecularL1 = tf.abs(tf.math.log(epsilon + deprocess(targets[0,:,:,9:12])) - tf.math.log(epsilon + deprocess(outputs[0,:,:,9:12]))) * a.specularLossFactor
            
            gen_loss_L1 = tf.reduce_mean(input_tensor=NormalL1 + DiffuseL1 + SpecularL1 + RoughnessL1)
        elif a.loss == "l2":
            epsilon = 0.001
            NormalL1 = tf.square(targets[0,:,:,0:3] - outputs[0,:,:,0:3]) * a.normalLossFactor
            DiffuseL1 = tf.square(tf.math.log(epsilon + deprocess(targets[0,:,:,3:6])) - tf.math.log(epsilon + deprocess(outputs[0,:,:,3:6]))) * a.diffuseLossFactor
            RoughnessL1 = tf.square(targets[0,:,:,6:9] - outputs[0,:,:,6:9]) * a.roughnessLossFactor
            SpecularL1 = tf.square(tf.math.log(epsilon + deprocess(targets[0,:,:,9:12])) - tf.math.log(epsilon + deprocess(outputs[0,:,:,9:12]))) * a.specularLossFactor

            gen_loss_L1 = tf.reduce_mean(input_tensor=NormalL1 + DiffuseL1 + SpecularL1 + RoughnessL1)        
        elif a.loss == "render" or a.loss == "renderL2":
            with tf.compat.v1.name_scope("renderer"):
                with tf.compat.v1.name_scope("diffuse"):
                ...
                with tf.compat.v1.name_scope ("specular"):
                ...
                gen_loss_L1 = 0
                if a.loss == "render":
                    gen_loss_L1 = tf.reduce_mean(input_tensor=tf.abs(tf.math.log(rerenderedTargets + 0.01) - tf.math.log(rerenderedOutputs + 0.01)))
                if a.loss == "renderL2":
                    gen_loss_L1 = tf.reduce_mean(input_tensor=tf.square(tf.math.log(rerenderedTargets + 0.01) - tf.math.log(rerenderedOutputs + 0.01)))
        consistencyLoss = 0
                    
        gen_loss = gen_loss_L1 * a.l1_weight

    with tf.compat.v1.name_scope("generator_train"):
        with tf.compat.v1.variable_scope("generator_train0", reuse=reuse_bool):
...
def main():
    ...
    '''
    ________________________________________________________________TF lite test
    '''

    if a.mode == 'eval_lite':

        def convert_test(image, squeeze=False):
            ...
            if squeeze:
                def tempLog(imageValue):                    
                    imageValue= tf.math.log(imageValue + 0.01)
                    imageValue = imageValue - tf.reduce_min(input_tensor=imageValue)
                    imageValue = imageValue / tf.reduce_max(input_tensor=imageValue)
                    return imageValue
        ...
        converted_inputs = convert_test(inputs_reshaped)
    ...
    with tf.compat.v1.name_scope("transform_images"):
    ...
    with tf.compat.v1.name_scope("outputs_summary"):
        tf.compat.v1.summary.image("outputs", converted_outputs, max_outputs=a.nbTargets)

    tf.compat.v1.summary.scalar("generator_loss", model.gen_loss_L1)
    ...
    with sv.managed_session() as sess:
        ...
        if a.mode == "test" or a.mode == "eval":
            ...
            _______________________________________________________________Test saving model_________________________________________________________________________
            ...
            def testTTT():
                ...
                converter = tf.compat.v1.lite.TFLiteConverter.from_session(sess, input_tensors=[examples.inputs], output_tensors=[model.outputs]);
...
def tf_Normalize(tensor):
    Length = tf.sqrt(tf.reduce_sum(input_tensor=tf.square(tensor), axis = -1, keepdims=True))
...
def tf_DotProduct(tensorA, tensorB):       
    return tf.reduce_sum(input_tensor=tf.multiply(tensorA, tensorB), axis = -1, keepdims=True)
